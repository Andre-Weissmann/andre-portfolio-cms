...
function renderDrawer(key) {
  ...
  /* Clean header: badge + title + subtitle + GitHub. No toolbar in header at all. */
  ...
  /* ── Sticky bottom bar: close + theme toggle ── */
  /* Lives outside the scroll area so it never overlaps content */
  var bottomBar = document.createElement('div');
  bottomBar.id = 'dd-bottom-bar';
  bottomBar.innerHTML =
    '<button id="dd-close" onclick="closeDD()" aria-label="Close panel">&#x2715; Close</button>' +
    '<button id="dd-theme-toggle" onclick="toggleDDTheme()" aria-label="Toggle light / dark" title="Toggle reading mode"><span id="dd-theme-icon">&#9790;</span> Dark</button>';
...
window.openDD = function(key) {
  ...
  if (!panel) return;
  renderDrawer(key);
  /* Restore saved theme preference (icon lives in bottom bar built by renderDrawer) */
  var savedTheme = 'dark';
  ...
  if (savedTheme === 'light') {
    ...
    if (themeBtn) themeBtn.innerHTML = '\u2600 Light';
  } else {
