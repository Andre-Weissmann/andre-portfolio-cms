...
function renderDrawer(key) {
  ...
  /* Row 1: badge + github only — toolbar injected AFTER slide-in animation ends */
  ...
  var topRow = '<div class="brief-hdr-top">' +
    '<div class="brief-hdr-left">' +
      '<span class="brief-badge" style="background:' + p.badgeColor + '">' + p.badge + '</span>' +
    ...
    '</div>' +
    '<!-- toolbar injected after animation -->' +
  '</div>';
  /* Row 2: title + subtitle + github (github hidden on desktop, shown on mobile) */
  var titleRow = '<div class="brief-hdr-titles">' +
    '<h2 class="brief-hdr-title">' + p.title + '</h2>' +
    '<p class="brief-hdr-sub">' + p.subtitle + '</p>' +
    (p.github ? '<a class="brief-github-btn brief-github-mobile" href="' + p.github + '" target="_blank" rel="noopener noreferrer" aria-label="View on GitHub" onclick="var w=window.open(this.href,\"_blank\",\"noopener,noreferrer\");if(!w){try{window.top.location.href=this.href;}catch(e){}}return false;">' + githubSvg + '<span class="brief-github-label">View on GitHub</span></a>' : '') +
  '</div>';
  hdr.innerHTML = topRow + titleRow;
...
window.toggleDDTheme = function() {
  ...
  try { sessionStorage.setItem('dd-theme', isLight ? 'light' : 'dark'); } catch(e) {}
};
...
window.openDD = function(key) {
  ...
  var savedTheme = 'dark';
  try { savedTheme = sessionStorage.getItem('dd-theme') || 'dark'; } catch(e) {}
  var icon = document.getElementById('dd-theme-icon');
  if (savedTheme === 'light') {
  ...
  /* Inject toolbar AFTER slide-in animation (420ms) so it never overlaps content mid-animation */
  setTimeout(function() {
    var topRow = document.querySelector('.brief-hdr-top');
    if (!topRow || document.getElementById('dd-panel-toolbar')) return;
    var toolbar = document.createElement('div');
    toolbar.id = 'dd-panel-toolbar';
    toolbar.style.cssText = 'opacity:0;transition:opacity 0.18s ease;';
    toolbar.innerHTML =
      '<button id="dd-theme-toggle" aria-label="Toggle reading mode" title="Toggle light / dark" onclick="toggleDDTheme()"><span id="dd-theme-icon">&#9790;</span></button>' +
      '<button id="dd-close" onclick="closeDD()" aria-label="Close">&times;</button>';
    topRow.appendChild(toolbar);
    /* Restore theme icon */
    var savedTheme = 'dark';
    try { savedTheme = sessionStorage.getItem('dd-theme') || 'dark'; } catch(e) {}
    var icon2 = document.getElementById('dd-theme-icon');
    if (savedTheme === 'light' && icon2) icon2.innerHTML = '&#9728;';
    /* Fade in */
    requestAnimationFrame(function() {
      requestAnimationFrame(function() { toolbar.style.opacity = '1'; });
    });
  }, 430);
};
